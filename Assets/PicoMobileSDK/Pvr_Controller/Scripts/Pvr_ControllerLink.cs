...
public class Pvr_ControllerLink
{
    ...
    {
        Debug.Log("PvrLog StartReceiver");
#if ANDROID_DEVICE
        Pvr_UnitySDKAPI.System.UPvr_CallStaticMethod(javaPico2ReceiverClass, "startReceiver",activity, gameobjname);
#endif
    }
    ...
    public Vector3 GetVelocity(int num)
    {
        ...
        if (PLOG.logLevel > 2)
        {
            PLOG.D("PvrLog Velocity" + velocity[0] / SWITCH_METER + "," + velocity[1] / SWITCH_METER + "," + velocity[2] / SWITCH_METER);
        }
        return new Vector3(velocity[0] / SWITCH_METER, velocity[1] / SWITCH_METER, -velocity[2] / SWITCH_METER);
    }
    ...
    public Vector3 GetAcceleration(int num)
    {
        ...
        if (PLOG.logLevel > 2)
        {
            PLOG.D("PvrLog Acce:" + acceData[0] / SWITCH_METER + acceData[1] / SWITCH_METER + acceData[2] / SWITCH_METER);
        }
        if (!float.IsNaN(acceData[0]) && !float.IsNaN(acceData[1]) && !float.IsNaN(acceData[2]))
        {
            return new Vector3(acceData[0] / SWITCH_METER, acceData[1] / SWITCH_METER, -acceData[2] / SWITCH_METER);
        }
        return new Vector3(0, 0, 0);
    }
